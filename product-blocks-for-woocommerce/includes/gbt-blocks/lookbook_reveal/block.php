			...
			<div class="gbt_18_distorsion_lookbook">
				<?php foreach ( $products as $product ) : ?>
					<section class="gbt_18_distorsion_lookbook_item">
						...
						?>
						<style>.product-<?php echo esc_attr( $product->get_id() ); ?> a.added_to_cart.wc-forward { color: <?php echo esc_attr( $textColor ); ?>; } ?> </style>
						<a class="gbt_18_fade_images_link" href="<?php echo esc_url( $product->get_permalink() ); ?>">
								<div class="gbt_18_fade_images">
									<?php echo isset( $iobj ) ? wp_kses_post( $iobj ) : wp_kses_post( wc_placeholder_img() ); ?>
									<?php echo isset( $iobj2 ) ? wp_kses_post( $iobj2 ) : ''; ?>
								</div>
						</a>
						<div class="gbt_18_distorsion_lookbook_content product-<?php echo esc_attr( $product->get_id() ); ?>" style="background-color: <?php echo esc_attr( $bgColor ); ?>;">
							<div class="gbt_18_text_wrapper" style="color:<?php echo esc_attr( $textColor ); ?>">
								<div class="gbt_18_content_top">
									<h2 style="color:<?php echo esc_attr( $textColor ); ?>"><?php echo esc_html( $product->get_name() ); ?></h2>
									<div class="gbt_18_description" style="color:<?php echo esc_attr( $textColor ); ?>"><?php echo wp_kses_post( $product->get_short_description() ); ?></div>
								</div>
								<div class="gbt_18_content_bottom">
									<span class="gbt_18_product_price" style="color:<?php echo esc_attr( $textColor ); ?>">
										<?php echo wp_kses_post( $product->get_price_html() ); ?>
									</span>
									<?php if ( ! function_exists( 'blockshop_version' ) ) : ?>
									<a 		style="color:<?php echo esc_attr( $bgColor ); ?>; background-color: <?php echo esc_attr( $textColor ); ?>;"
									...
									<a 		style=""
										...
										<style type="text/css">
										.product-<?php echo esc_attr( $product->get_id() ); ?> a.blockshop_add_to_cart.single_add_to_cart_button.button.alt,
										.product-<?php echo esc_attr( $product->get_id() ); ?> a.added_to_cart.wc-forward,
										.gbt_18_distorsion_lookbook .gbt_18_distorsion_lookbook_content.product-<?php echo esc_attr( $product->get_id() ); ?> .gbt_18_description a{
										...
										.product-<?php echo esc_attr( $product->get_id() ); ?> a.blockshop_add_to_cart.single_add_to_cart_button.button.alt.loading {
											border-color: <?php echo esc_attr( $textColor ); ?>;
										}
										.product-<?php echo esc_attr( $product->get_id() ); ?> a.blockshop_add_to_cart.single_add_to_cart_button.button.alt.loading:after {
											background-color: <?php echo esc_attr( $textColor ); ?>
										}
